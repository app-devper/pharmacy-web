...
export const getExpiringLots = (days = 60) =>
  apiFetch<ExpiringLot[]>(`/api/pharmacy/v1/lots/expiring?days=${days}`)

export const getExpiredLots = () =>
  apiFetch<ExpiringLot[]>(`/api/pharmacy/v1/lots/expiring?expired_only=true`)

export const writeoffLots = (lotIds: string[]) =>
  apiFetch<{ written_off: number }>('/api/pharmacy/v1/lots/writeoff', {
    method: 'POST',
